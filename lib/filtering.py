...
def subtract_channels(trial_data, deoxy_const=None):
    ...
    for ch in range(0, 16, 2):    
        ...
        new_col_label = strings.column_names(ch, num_channels=8)
...
def cut_segments(trial, time = 15):
    """Resample segments to predefined length, then subtract the mean."""

    target_len = time * int(trial.sample_rate) # default sample rate is 25Hz, for 20 seconds
    normalized = []
